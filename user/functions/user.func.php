...
add_action('user.update', function($arg){
	if(!empty($arg['user_password']) && !is_admin()){
		...
		$pwd = database::open(0)->select('user', 'user_password', "`user_id` = {$arg['user_id']}")->fetchObject()->user_password;
		if(!hash_verify($pwd, $arg['old_password'])) return error(lang('user.wrongOldPassword'));
	}
});

/** 检查是否开启用户注册 */
add_hook('user.add', function(){
	if(!config('user.register.enable') && !is_admin()) return error(lang('user.regDisabled'));
});
